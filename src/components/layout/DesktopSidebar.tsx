...
const NAV_ITEMS: Array<{
...
}> = [
  {
    label: '홈', href: '/home',
    icon: (active: boolean) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? 'var(--color-primary)' : 'none'} stroke={active ? 'var(--color-primary)' : 'var(--theme-text-subtle)'} strokeWidth="2" strokeLinecap="round">
  ...
  },
  {
    ...
    icon: (active: boolean) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? 'var(--color-primary)' : 'var(--theme-text-subtle)'} strokeWidth="2" strokeLinecap="round">
        ...
        {active && <circle cx="16" cy="15" r="1" fill="var(--color-primary)" stroke="none"/>}
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (active: boolean) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? 'rgba(0,132,204,0.12)' : 'none'} stroke={active ? 'var(--color-primary)' : 'var(--theme-text-subtle)'} strokeWidth="2" strokeLinecap="round">
...
export function DesktopSidebar() {
  ...
  return (
    <aside style={{
      ...
      boxShadow: '1px 0 0 var(--theme-border)',
      zIndex: 50,
          ...
          return (
            <Link
              ...
              style={{
                ...
                background: isActive ? 'var(--theme-selected-bg)' : 'transparent',
                color: isActive ? 'var(--color-primary)' : 'var(--theme-text-subtle)',
                position: 'relative',
              }}
            >
              {/* 왼쪽 액티브 인디케이터 */}
              ...
              {isActive && (
                <div style={{ marginLeft: 'auto', width: '6px', height: '6px', borderRadius: '50%', background: 'var(--color-primary)', flexShrink: 0 }} />
              )}
            </Link>
          );
        })}
      ...
      <div style={{
        ...
        <div style={{ overflow: 'hidden', flex: 1 }}>
          ...
          <p style={{ fontSize: '11px', color: 'var(--theme-text-subtle)', fontWeight: 600, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            마이페이지 설정
          </p>
        </div>
      </div>
