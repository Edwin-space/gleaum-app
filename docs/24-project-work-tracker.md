...
| 4 | `IOS-010` | 일정·공간·가계부·알림·전체 메뉴 네이티브화 | `🟠 공간 1차 완료·가계부 대기` | 2026-07-28 일정·공간 탭 WebView 제거. 다음은 개인 가계부를 Apple 시스템 패턴으로 구현 |
...
| [ ] | `IOS-010` | 핵심 기능 네이티브화 | `🟠 일정·공간 1차 완료·가계부 대기` | 2026-07-27 | — | 홈·온보딩·일정에 이어 공간 목록/전환, 다가오는 일정, 소식 작성, 멤버 권한·가족 관계, 코드 초대·공유, 공간 생성·참여·이름 변경·가족 승격·안전 삭제를 SwiftUI `List`·`Form`·toolbar·menu·sheet·confirmation dialog로 구현했다. 공간 탭 WebView 폴백을 제거하고 시작 snapshot 디코딩, mutation 즉시 반영, 관련 home+schedules 선택 갱신을 연결했다. 개인 공간에는 커뮤니티·초대·관리 액션을 노출하지 않는다. 실제 계정 역할별 CRUD는 `IOS-001`; 다음은 개인 가계부→알림/전체 순서 |
| [ ] | `IOS-011` | iPhone·테마·접근성 QA | `🟠 화면별 진행 중·iPad 후순위` | 2026-07-28 | — | 일정 목록·상세·폼과 공간 메인 화면을 iPhone 17 Pro 라이트/다크, 접근성 최대 글자로 검증했다. 일정은 큰 글자에서 행 세로 전환·유형 필터 메뉴 전환, 공간은 역할 메타데이터 세로 전환을 적용했다. 나머지 핵심 화면과 소형/대형 iPhone, Reduce Motion·오프라인/세션·VoiceOver 실음성 회귀가 남음. iPad·Split View는 후순위 |
