...
| [ ] | `IOS-011` | iPhone·테마·접근성 QA | `🟠 실제 iPhone 제스처·하단 액션 보정 완료` | 2026-07-28 | — | 실제 iPhone 16 Pro에서 5개 탭 이동, 공간 목록의 아래 스크롤 축소·반대 스크롤 복원, 시스템/라이트/다크와 상태바 전환을 확인했다. 기기 일정 가져오기처럼 고정 하단 액션이 있는 상세 화면에서는 플로팅 탭 바를 숨기고 뒤로 이동하면 복원해 버튼 충돌을 제거했다. 접근성 트리는 4/5탭 각각 독립 버튼·선택 상태, 가져오기 버튼 노출을 확인했다. 소형/대형 iPhone, Reduce Motion·VoiceOver 실음성, 오프라인/세션 회귀가 남음. iPad·Split View는 후순위 |
