...
| [ ] | `IOS-010` | 핵심 기능 네이티브화 | `🟠 홈·온보딩 1차·나머지 대기` | 2026-07-27 | — | 홈은 snapshot 기반 SwiftUI·시스템 toolbar·pull-to-refresh·기존 네이티브 일정 생성 sheet 연결까지 구현. 온보딩은 이름/표시 방식→사용 목적→홈 구성→알림의 4단계 SwiftUI 흐름과 완료 API 연동을 구현했다. 일정→공간→가계부→알림/전체 순서로 WebView 없이 구현하고 각 완료 시 폴백 경로를 제거 |
