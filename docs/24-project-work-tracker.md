...
| 7 | `IOS-011` | iPhone·테마·접근성 QA | `🟠 핵심 화면·내비게이션 보정 완료·기기 회귀 대기` | 화면 모드의 시스템/라이트/다크 식별 색과 양쪽 테마 미리보기를 분리하고, iOS 26 시스템 탭 바 스크롤 축소와 5개 루트 Large Title 규칙을 적용했다. 다음은 실제 손가락 스크롤·소형/대형 iPhone, Reduce Motion·VoiceOver 실음성·오프라인/세션 회귀이며 iPad·Split View는 후순위 |
...
| [ ] | `IOS-002` | EventKit 캘린더 UX | `🟠 코드·시뮬레이터 완료·실기기 회귀 대기` | 2026-07-28 | — | 전체 접근 권한·쓰기 가능 캘린더 선택, 글리움→iPhone 30일 수동 동기화, iPhone→개인 일정 선택 가져오기, `gleaum:schedule:{id}` 소유 마커와 제목+시작시각 60초 중복 정책을 SwiftUI로 구현했다. 권한은 연결 상태로 분리하고 대상 캘린더·계정·양방향 작업은 단일 `일정 동기화` 섹션에 통합했다. 마커 없는 기기 일정은 수정·삭제하지 않는다. Debug·Release와 라이트·다크·큰 글자 화면 QA 통과. 실제 iPhone에서 권한 거절/허용, 생성·수정·삭제·중복 가져오기를 회귀한 뒤 완료 처리 |
...
| [ ] | `IOS-011` | iPhone·테마·접근성 QA | `🟠 핵심 화면·내비게이션 보정 완료·기기 회귀 대기` | 2026-07-28 | — | 화면 모드 선택 행에 시스템=Blue, 라이트=Orange, 다크=Indigo 식별색을 적용하고 미리보기는 현재 앱 테마와 무관하게 양쪽 팔레트를 동시에 표시한다. 5개 루트 탭은 Apple Large Title로 통일했다. iOS 26 이상은 공식 `tabBarMinimizeBehavior(.onScrollDown)`로 콘텐츠 스크롤 시 시스템 탭 바가 축소되며 iOS 15~25는 접근성과 안정성을 위해 시스템 기본 탭 바를 유지한다. Debug·Release Simulator build와 라이트·다크 화면 확인 통과. 실제 손가락 스크롤, 소형/대형 iPhone, Reduce Motion·오프라인/세션·VoiceOver 실음성 회귀가 남음. iPad·Split View는 후순위 |
...
|---|---|---|---|---|
| 2026-07-28 | `IOS-009`, `IOS-011` | iOS 테마·탭 바·루트 제목 체계 보정 | 화면 모드의 inline Picker를 시스템/라이트/다크별 의미색·설명·선택 상태가 분명한 Apple Form 행으로 교체했다. 현재 창의 테마에 동적으로 묶여 라이트/다크 예시가 같은 모습으로 보이던 문제를 제거하고 두 팔레트를 명시적으로 동시에 렌더링한다. 홈·일정·공간·가계부·전체는 모두 Large Title 규칙으로 통일했다. iOS 26 이상에는 Apple 공식 탭 바 스크롤 축소를 적용하고 iOS 15~25는 시스템 기본 탭 바를 유지한다 | iPhone 17 Pro iOS 26.5에서 라이트·다크 선택 상태와 양쪽 미리보기, 전체·일정 Large Title의 스크롤 축소를 확인했다. Debug·Release Simulator build와 diff check 통과. Computer Use의 접근성 Scroll 액션은 실제 터치 제스처가 아니므로 탭 바 축소 애니메이션은 실제 iPhone 손가락 스크롤에서 최종 회귀한다 |
