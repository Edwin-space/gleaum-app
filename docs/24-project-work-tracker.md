...
| 순서 | ID | 작업 | 상태 | 다음 행동 |
|---:|---|---|---|---|
| 1 | `FAM-011` | 보호자 이메일 OTP·동의 정합화 | `🟠 실메일 회귀 대기` | Production `dpl_3M2He5p9F3UfBs5H4tW3u7kRXZwy`·운영 DB·Supabase Auth 템플릿 적용 완료. 새 코드 요청→OTP 입력→필수 동의→초대 준비 완료 확인 |
...
| [ ] | `FAM-011` | 보호자 이메일 OTP·필수 동의 정합화 | `🟠 실메일 회귀 대기` | 2026-07-23 | — | 실메일 8자리에 맞춰 OTP UI/API/공용 템플릿을 통일하고 확인 증적 DB 강제, `email_otp` 이력 완료. 보호자 전용 `emailRedirectTo`와 조건 분기형 Magic Link/OTP 공용 템플릿을 운영 Dashboard에 적용. 8자리 수정 Production 배포 후 보호자 본문·코드·필수 동의 완료 확인 필요 |
...
|---|---|---|---|---|
| 2026-07-23 | `FAM-011`, `OPS-004` | 운영 OTP 길이 정합화 | 앱과 문서는 6자리로 제한했지만 운영 Supabase 실메일의 `{{ .Token }}`은 8자리로 발급되는 불일치를 확인. `GUARDIAN_EMAIL_OTP_LENGTH=8` 단일 상수로 UI 입력·버튼 상태·API 형식 검증을 통일하고 메일 템플릿 안내도 8자리로 변경 | 대상 ESLint·production build·운영 템플릿 저장·Production 배포 후 새 코드 요청→8자리 입력→동의 완료 회귀 |
