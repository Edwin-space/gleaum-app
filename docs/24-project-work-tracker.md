...
| 2026-08-04 | `AUTH-002`, `IOS-008` | 카카오 로그인 Supabase OAuth 단일화·중복 콜백 차단 | Supabase Auth 로그 분석 결과를 반영해 Kakao Native ID Token(`signInWithIdToken`)의 `aud`(Native App Key)와 Supabase Kakao Client ID(`REST API Key`) 불일치 원인을 진단하고, 카카오 로그인을 Supabase OAuth(`auth/v1/authorize?provider=kakao`) 단일 방식으로 통일했다. iOS `AppDelegate.swift`에서 `ASWebAuthenticationSession` 처리 시 발생하는 중복 authorization code 교환(`invalid_grant`)을 `!SessionManager.shared.hasValidSession()` 검사로 차단하여 카카오 로그인 실패를 근본적으로 수정했다 | xcodebuild generic iOS simulator `BUILD SUCCEEDED` 4회 검증, `git diff --check`, Vercel production deployment 승격 완료 |
