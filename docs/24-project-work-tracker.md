...
| [ ] | `IOS-011` | iPhone·테마·접근성 QA | `🟠 핵심 화면·적응형 내비게이션 보정 완료·기기 회귀 대기` | 2026-07-28 | — | 화면 모드 선택 행에 시스템=Blue, 라이트=Orange, 다크=Indigo 식별색을 적용하고 양쪽 팔레트를 독립 표시한다. 5개 루트는 Large Title로 통일했다. 사용자 제공 Instagram iOS 영상 기준으로 iOS 18 이상에 적응형 플로팅 탭 바를 구현했다. 아래 방향 콘텐츠 이동에서는 전체 메뉴를 유지한 채 캡슐 폭·높이·아이콘을 축소하고 레이블만 숨기며, 반대 방향·목록 상단·탭 전환에서는 확장한다. 접근성 글자 크기에서는 레이블을 보존하고 Reduce Motion을 따른다. iOS 15~17은 시스템 탭 바 fallback이다. Debug·Release와 4탭/5탭 확장→축소→복원 시각 QA 통과. 실제 iPhone 손가락 제스처, 소형/대형 iPhone, 오프라인/세션·VoiceOver 실음성 회귀가 남음. iPad·Split View는 후순위 |
