...
| 3 | `IOS-009` | SwiftUI 단일 앱 셸·라우터·선조회 | `🟠 1차 기반 구현·iPhone 검증` | SwiftUI를 실제 window root로 전환하고 브랜드 전환·병렬 snapshot·시스템 5탭·홈·일정·공간 1차 구현을 완료했다. 다음은 가계부→알림/전체 순서로 웹 폴백을 제거하고 탭별 `NavigationStack` 상태를 완성 |
| 4 | `IOS-010` | 일정·공간·가계부·알림·전체 메뉴 네이티브화 | `🟠 일정·공간·가계부 완료` | 개인 가계부 WebView를 제거하고 수입/지출·월간 흐름·카테고리·반복 예정·CRUD·상태 변경을 네이티브화했다. 다음은 알림→전체 메뉴 순서 |
...
| 7 | `IOS-011` | iPhone·테마·접근성 QA | `🟠 화면별 진행 중` | 일정·공간 화면 iPhone 17 Pro 라이트·다크·접근성 큰 글자 확인 완료. 나머지 핵심 화면과 소형/대형 iPhone 회귀, iPad·Split View는 후순위 |
...
| [ ] | `IOS-010` | 핵심 기능 네이티브화 | `🟠 일정·공간·가계부 완료` | 2026-07-27 | — | 홈·온보딩·일정·공간에 이어 개인 가계부 탭의 WebView 폴백을 제거했다. 개인 원장 전용 API로 월 이동, 순수익·수입·지출·예정 요약, 카테고리 필터, 검색, 정기 항목, 생성·상세·수정·삭제·상태 변경을 SwiftUI `List`·`Form`·toolbar·menu·sheet·confirmation dialog에 연결했다. capability가 가계부를 제한하는 자녀 계정은 탭 자체를 노출하지 않으며 공유 공간 원장은 선택할 수 없다. 시작 snapshot 캐시와 변경 뒤 현재 월+홈 선택 갱신을 사용한다. 실제 계정 역할별 CRUD는 `IOS-001`; 다음은 알림→전체 메뉴 순서 |
| [ ] | `IOS-011` | iPhone·테마·접근성 QA | `🟠 일정·공간·가계부 통과·iPad 후순위` | 2026-07-28 | — | 일정·공간·가계부를 iPhone 17 Pro 라이트/다크와 접근성 최대 글자에서 검증했다. 가계부는 의미 기반 색상, SF Symbols, 큰 글자 행 재배치와 화면 최대 Dynamic Type 범위를 적용해 금액·월 선택·검색이 단일 문자 열로 붕괴하지 않게 보정했다. 나머지 핵심 화면과 소형/대형 iPhone, Reduce Motion·오프라인/세션·VoiceOver 실음성 회귀가 남음. iPad·Split View는 후순위 |
