...
| 4 | `IOS-010` | 일정·공간·가계부·알림·전체 메뉴 네이티브화 | `🟠 일정 완료·공간 대기` | 2026-07-28 일정 목록·상세·생성/수정/삭제와 캐시 갱신을 Apple 시스템 패턴으로 구현하고 일정 탭 WebView 폴백 제거. 다음은 공간 목록·상세·멤버·초대 |
...
| 7 | `IOS-011` | iPhone·테마·접근성 QA | `🟠 화면별 진행 중` | 일정 화면 iPhone 17 Pro 라이트·다크·접근성 큰 글자·VoiceOver 요소 확인 완료. 나머지 핵심 화면과 소형/대형 iPhone 회귀, iPad·Split View는 후순위 |
...
| [ ] | `IOS-010` | 핵심 기능 네이티브화 | `🟠 일정 완료·공간 대기` | 2026-07-27 | — | 홈·온보딩에 이어 일정 목록/검색/유형·기간 필터, 상세, 생성·수정·상태 변경·삭제를 SwiftUI `List`·`Form`·toolbar·sheet·confirmation dialog로 구현했다. 일정 탭 WebView 폴백을 제거하고 시작 snapshot 일정 디코딩, 변경 즉시 반영, home+schedules 선택 갱신을 연결했다. 다음은 공간→가계부→알림/전체 순서 |
| [ ] | `IOS-011` | iPhone·테마·접근성 QA | `🟠 화면별 진행 중·iPad 후순위` | 2026-07-28 | — | 일정 목록·상세·폼을 iPhone 17 Pro 라이트/다크, 접근성 큰 글자, VoiceOver 요소로 검증하고 큰 글자에서 일정 행 세로 전환·유형 필터 메뉴 전환을 적용했다. 나머지 핵심 화면과 소형/대형 iPhone, Reduce Motion·오프라인/세션 회귀가 남음. iPad·Split View는 후순위 |
