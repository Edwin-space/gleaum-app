...
| 기기 일정 가져오기 | 92 | A | WebView 제거, Filled 안내·ListItem 선택 목록·고정 CTA·시스템 인셋 적용 | `SM_F731N` 후보 3개 조회, 잘림·겹침·크래시 없음 |
...
## 6. 2026-07-16 적응형·접근성 부분 QA

| 항목 | 결과 | 근거 |
|---|---|---|
| Compact 세로 | 통과 | `SM_F731N`, 1080×2640, NavigationBar·앱바·카드·FAB 정상 |
| 글꼴 1.3배 | 통과 | 제목·본문·내비게이션 라벨 잘림·겹침 없음 |
| 다크 테마 | 통과 | 텍스트·카드·아이콘·시스템 바 대비 정상 |
| Expanded/가로 | 통과 | 2560×1600 / 320dpi, NavigationRail과 840dp 콘텐츠 중앙 정렬 확인 |
| 접근성 의미·터치 영역 | 통과 | UIAutomator content description과 44dp 이상 주요 클릭 영역 확인 |
| TalkBack 음성 탐색 | 대기 | 실제 음성 순서·초점 이동은 사용자 수동 확인 필요 |

expanded 첫 검증에서 `fillMaxSize()`가 `widthIn(max=…)`보다 먼저 부모 폭을 고정해 720/840dp 제한이 무효화되는 문제를 발견했다. 공통 `GleaumAdaptiveContent`의 Modifier 순서를 수정하고 같은 규격에서 재검증했다.

로그인/가입은 브랜드 고정 다크 XML을 유지한다. 1080×2640과 글꼴 1.3배에서 로고·태그라인·Google/이메일 버튼·법적 고지의 잘림과 겹침이 없고 접근성 계층이 제공되므로 2026-07-16 브랜드 예외로 승인했다. 기능 또는 정보 구조를 크게 변경할 때 Compose 전환을 다시 평가한다.

## 7. 2026-07-23 로그인 실기기 부분 QA

- `SM_F731N`에서 스플래시 이후 네이티브 홈과 홈/일정/공간/가계부/전체 하단 메뉴 왕복을 확인했다.
- 캘린더 설정 목록은 320dp 이내 스크롤, 긴 이름 한 줄 말줄임, 행 단위 `RadioButton` 접근성 역할로 보정했다.
- `기기 일정 가져오기`는 Compose Material 3 독립 Activity로 전환했으며 후보 3개 조회와 선택 상태를 확인했다.
- 최초 캡처에서 하단 CTA가 시스템 내비게이션과 겹치는 문제를 발견해 `navigationBarsPadding()` 적용 후 재검증했다.
- 실제 TalkBack 음성 순서와 캘린더 생성·재조회 중복 검증은 수동 QA로 남긴다.
