...
> 상태: migration `020`·`021`·`022` 운영 적용 및 스키마/RLS 검증 완료. 비용 최적화형 이메일 확인·최종 승인 UI 구현 완료
> 기준일: 2026-07-13
...
| `GET /api/session/context` | 기반 완료 | 플랫폼 공통 accountMode/capabilities 반환 |
...
화면 숨김만으로 끝내지 않고 추후 자녀 일정·가계부·위치 API와 RLS에서도 같은 capability를 검증해야 한다.
...
운영 `gleaum_app` 프로젝트에는 migration `020`·`021`·`022` 적용과 테이블·함수·RLS·Advisor 검증까지 완료했다.
...
2. Android/iOS/Web 공통 session context 적용
3. 자녀 전용 홈 및 메뉴 capability 차단
4. 일정 assignee/observer 모델과 RLS
5. 만 14세 본인 재동의 UI
