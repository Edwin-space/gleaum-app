...
- Web 기능을 이유 없이 삭제하거나 Android 전용 기능을 임의 추가하지 않는다.
...
- 기능 의미를 바꾸는 UX 개선을 겸하지 않는다.
- 화면별 자체 색상·그라데이션·하단 네비게이션을 만들지 않는다.
...
2. **Android Compose Material 3 화면 구현**
...
- `NativePortFlags`의 홈, 일정 목록/상세/폼, 가계부 목록/폼, 공간, 전체 메뉴, 알림, 온보딩 Compose gate가 모두 활성화되어 있다.
- 폰은 Material 3 NavigationBar, medium/expanded는 NavigationRail 계열로 전환한다.
- 로그인/가입은 아직 XML 기반이며 코드 감사 86/B로, Compose 전환 또는 브랜드 예외 승인 대상이다.
- 최종 실기기 시각 QA는 완료되지 않았다. `docs/20-android-native-release-qa.md`, `docs/22-android-material3-ui-audit.md`를 따른다.
...
> 이 절의 2026-06-22~23 비활성 preview 기록은 과거 이력이다. 현재 활성 상태는 4절의 `현재 활성 상태`와 `NativePortFlags.kt`를 기준으로 한다.
