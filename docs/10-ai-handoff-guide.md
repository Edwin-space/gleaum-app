...
| 날짜 | 내용 |
|------|------|
| 2026-07-28 | iPhone 16 Pro 실제 계정에서 EventKit 전체 접근·iCloud 캘린더 선택과 격리 일정 내보내기 생성 1건→수정 업데이트 1건→서비스 삭제 후 기기 정리 1건→재동기화 0건을 확인하고 QA 데이터를 제거했다. iPhone 일정 가져오기 상세에서는 플로팅 탭 바가 하단 액션을 덮지 않도록 상세 진입 중 숨기고 복귀 시 복원한다. EventKit은 동기화마다 저장소를 재조회하고 동일 `gleaum:schedule:{id}` 중복 이벤트를 대표 1개로 정리한다. Simulator fixture의 가져오기 3건·중복 1건·하단 버튼 노출과 Debug/Release build를 통과했다. 다음은 실제 기기 일정 1건 가져오기→재조회 중복 차단과 권한 거절 UX다. 단, 현재 로컬 Apple Development 인증서가 무효이며 codesigning identity가 0건이므로 Xcode Accounts에서 인증서를 갱신해야 새 실기기 빌드를 설치할 수 있다. |
| 2026-07-28 | iOS 알림 흐름을 네이티브화했다. 홈 toolbar의 읽지 않은 배지에서 SwiftUI 알림 센터를 열고 전체/읽지 않음 필터, 새/이전 섹션, 개별·전체 읽음, 일정 상세·공간 탭 이동을 제공한다. 설정 `Form`은 iOS 시스템 권한 상태·설정 앱 이동과 일정/루틴/가계부/공간 서버 수신 설정을 함께 관리한다. APNs/FCM 토큰 등록 API는 Cookie·Bearer 인증을 공통 지원하고 `profiles.fcm_token`과 `fcm_tokens`를 동기화한다. 알림 탭의 `url`·`link`·`deep_link`는 중앙 네이티브 라우터가 처리한다. iPhone 17 Pro iOS 26.5 라이트·다크·접근성 최대 글자, Simulator Debug·Release와 Next production build 55/55를 통과했다. 실제 푸시 송수신은 운영 API 배포와 유료 Apple Team의 APNs/Firebase/실기기 설정이 필요한 `IOS-003`이며 다음 화면은 전체 메뉴다. |
...
### 2026-07-28 추가 — iOS 가족·자녀 계정 연결 네이티브화

- 가족 공간의 `가족 초대`는 일반 가족 구성원과 자녀를 먼저 구분한다. 일반 가족은 기존 코드/링크, 자녀는 별도 보호자 확인 흐름으로 진입한다.
- 자녀 목록·DatePicker 등록, 보호자 이메일 8자리 OTP와 재발송, 서비스/개인정보/가족 공유 필수 동의, 72시간 일회성 초대 공유, 자녀 claim, 보호자 최종 승인/거절을 기존 운영 API 8개에 연결했다.
...
- 전체 메뉴의 `기기 캘린더`는 권한을 `연결 상태`로 분리하고, 쓰기 가능한 대상 캘린더·계정·`글리움 일정을 iPhone에 반영`·`iPhone 일정을 글리움에 추가`를 하나의 `일정 동기화` 섹션에 묶는다. 기술적인 방향 구분보다 사용 목적이 먼저 읽히도록 구성한다.
